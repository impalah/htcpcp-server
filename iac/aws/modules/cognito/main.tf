...
resource "aws_cognito_user_pool" "userpool" {
  ...
  name              = format("%s-%s-userpool", var.environment, var.userpool_name)
...
resource "aws_cognito_user_pool_client" "public_client" {
  name = format("%s-%s-public-client", var.environment, var.userpool_name)
...
resource "aws_cognito_user_pool_client" "api_client" {
  ...
  for_each = { for user in var.api_client_users : user.name => user }

  name                          = format("%s-%s-%s-api-client", var.environment, var.userpool_name, each.value.name)
  user_pool_id                  = aws_cognito_user_pool.userpool.id
  ...
  allowed_oauth_scopes         = [for scope in each.value.scopes : format("%s/%s", var.resource_server_identifier, scope)]
  supported_identity_providers = ["COGNITO"]
